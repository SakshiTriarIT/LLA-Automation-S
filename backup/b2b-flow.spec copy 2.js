...
test('B2B Flow', async ({ page, context }, testInfo) => {
  // Single folder for B2B (avoid Playwright appending project/test subfolder and duplicating b2b-flow-B2B-Flow)
  const outDir = path.join(__dirname, '..', 'test-results', 'b2b-flow-B2B-Flow');
 ...
 //   find(page, "button 'Create Contact'", () => page.getByRole('button', { name: 'Create Contact' }).click()));
  ...
  //urlWithCartId="https://cwc--qasales.sandbox.lightning.force.com/lightning/r/Account/001di00000kXT3HAAW/view?c__cartId=0Q0di000001iij7CAA"
  ...
  await runStep(page, 29, 'Add Product to Cart (Catalog + BYOD)', async () => {
    ...
    if (urlWithCartId) {
      ...
      log('  [29] TRACE: Waiting 12s for cart UI (and iframe) to render...');
      ...
      const findCartFrame = async () => {
        ...
        log('  [29] TRACE: Checking ' + (frames.length - 1) + ' child frame(s) for Catalog...');
      ...
      for (let attempt = 1; attempt <= 3 && !cartFrame; attempt++) {
    ...
    // Open catalog: try Add > Catalog first, else Catalog icon (aria-label="Catalog", vlocity-btn slds-button_icon)
    const clickCatalogOpen = async () => {
      const addCatalog = cartCtx.getByRole('button', { name: 'Add > Catalog' });
      const catalogIcon = cartCtx.getByRole('button', { name: 'Catalog', exact: true });
      if (await addCatalog.first().isVisible({ timeout: 5000 }).catch(() => false)) {
        await addCatalog.click(t);
      } else if (await catalogIcon.first().isVisible({ timeout: 3000 }).catch(() => false)) {
        await catalogIcon.click(t);
      } else {
        throw new Error('Neither Add > Catalog nor Catalog (icon) button visible');
      }
    };
    log('  [29] Opening catalog (Add > Catalog or Catalog icon)...');
    await find(page, "open catalog (Add > Catalog or Catalog)", clickCatalogOpen, step29Meta);
    await captureDetail(29, '04-catalog-opened');
    log('  [29] Catalog opened. Finding row Liberty Business On The Goand clicking Add Product in that row...');
    ...
    log('  [29] Add > Cart done. Opening catalog again...');
    await find(page, "open catalog (Add > Catalog or Catalog)", clickCatalogOpen, step29Meta);
    await captureDetail(29, '07-catalog-opened-again');
    log('  [29] Catalog opened. Waiting for category list to render...');
    await new Promise((r) => setTimeout(r, 2000));
    ...
    log('  [29] BYOD category clicked. Finding row BYOD Mobile Phone and clicking Add Product in that row...');
